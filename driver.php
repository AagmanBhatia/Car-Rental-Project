...
<?php
    ...
    if (isset($_POST['sign_btn'])){
        ...
        $license=mysqli_real_escape_string($config,$_POST['license']);
        ...
        if (empty($firstname)){
            ...
            $check_email="SELECT * FROM driver WHERE email='$email'";
            $data=mysqli_query($config,$check_email);
            $result=mysqli_fetch_array($data);

            $check_contact="SELECT * FROM driver WHERE contact='$contact'";
            ...
            if($result>0){
                ...
                $pass=md5($password);
                $insert="INSERT INTO driver (firstname,lastname,gender,contact,license,address,email,password) VALUES('$firstname','$lastname','$gender','$contact','$license','$address','$email','$password')";
